...
$do=isset($_GET['do'])?daddslashes($_GET['do']):'0';
if($do!='0'&&$do!='1'&&$do!='2'&&$do!='3'&&$do!='4'&&$do!='5'&&$do!='6')exit('参数错误！');
...
<div class="panel panel-primary">
	<div class="panel-heading" style="background: #15A638;">
		<h3 class="panel-title" align="center">小白互赞宝程序安装</h3>
	</div>
	<div class="panel-body">
		...
		<?php }else{?>
		<p align="center"><a class="btn btn-success" href="index.php?do=1">开 始 安 装</a> <a class="btn btn-info" href="https://jq.qq.com/?_wv=1027&k=5jSaUbU">查看演示站</a></p>
		<?php }?>
	</div>
</div>

<script>layer.alert('<font color="red">本程序的服务范围仅为互赞，请勿将用户数据进行贩卖或用作其它用途，若有发现必将配合有关部门追查到底</font>',{'title':'警告'});</script>
<?php }elseif($do=='1'){?>
<div class="panel panel-primary">
...
if(defined("SAE_ACCESSKEY") || $_GET['jump']==1){
	...
	if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
		...
		if(!$con=DB::connect($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port'])){
			...
			if(DB::query("select * from zan_config where 1")==FALSE)
				echo '<p align="right"><a class="btn btn-primary btn-block" href="?do=4">创建数据表>></a></p>';
			else
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过香程互赞宝程序</div>
	...
	$db_host=isset($_POST['db_host'])?daddslashes($_POST['db_host']):NULL;
	$db_port=isset($_POST['db_port'])?daddslashes($_POST['db_port']):NULL;
	$db_user=isset($_POST['db_user'])?daddslashes($_POST['db_user']):NULL;
	$db_pwd=isset($_POST['db_pwd'])?daddslashes($_POST['db_pwd']):NULL;
	$db_name=isset($_POST['db_name'])?daddslashes($_POST['db_name']):NULL;
	$db_qz=isset($_POST['db_qz'])?daddslashes($_POST['db_qz']):'shua';
...
?>";
		if(!$con=DB::connect($db_host,$db_user,$db_pwd,$db_name,$db_port)){
			...
			if(DB::query("select * from zan_config where 1")==FALSE)
				...
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过小白互赞宝程序</div>
				<div class="list-group-item">
					<a href="?do=6" class="btn btn-block btn-info">跳过安装</a>
				</div>
				<div class="list-group-item">
